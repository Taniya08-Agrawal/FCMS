...
if ($_SERVER['REQUEST_METHOD']=='POST') {
  if (isset($_POST['update'])) {
    ...
    $deitcian_name=$_POST['deitcian_name'];
    $Contact_no=$_POST['Contact_no'];
    $address=$_POST['address'];
    $DOB=$_POST['DOB'];
    $Gender=$_POST['Gender'];
    $sql="UPDATE deitcian SET deitcian_name='$deitcian_name', Contact_no='$Contact_no', address='$address', DOB='$DOB', Gender='$Gender' WHERE deitcian_id='$deitcian_id'";
...
<?php
  ...
  if (isset($_POST['delete'])) {
    $deitcian_id=$_POST['deitcian_id'];
    $sql="DELETE FROM deitcian WHERE deitcian_id='$deitcian_id'";
    $run=mysqli_query($db, $sql);
      if ($run) {
?>
      <p style="font-size: 18px; font-family: cursive; color:#004d00;">
...
?>
    ...
    <label for="condition">
      <b style="font-size: 20px; font-family: cursive; color:#004d00;">Sort by:</b> 
    </label>
    <select name="condition" required>
      <option value="deitcian_name"> <b style="font-size: 20px; font-family: cursive; color:#004d00;"></b>Name</b></option>
      <option value="salary"> <b style="font-size: 20px; font-family: cursive; color:#004d00;"></b>Salary</b></option>
      <option value="DOB"> <b style="font-size: 20px; font-family: cursive; color:#004d00;"></b>DOB</b></option>
      <option value="Gender"> <b style="font-size: 20px; font-family: cursive; color:#004d00;"></b>Gender</b></option>
    </select>
  ...
  --- OR ---
  <br>
  <b style="font-size: 18px; font-family: cursive; color:#004d00;">View and update details for:</b>
  <form action="" method="POST">
    <label for="C_ID">
      <b style="font-size: 20px; font-family: cursive; color:#004d00;">Enter deitcian ID:</b>
    </label>
    <input type="text" name="deitcian_id">
...
?>
      <table>
      <tr>
        <td style="font-size: 20px; font-family: cursive; color:#004d00; padding:15px 15px 15px 15px;">deitcian ID</td>
...
<?php
      ...
      //all centres
      if($centre=="all"){
        $sql="SELECT * FROM deitcian ORDER BY '$sort'";
        $run=mysqli_query($db, $sql);
          $arr=mysqli_fetch_array($run);
          //check query run
          if($arr){
...
?>
          <tr>
            <form action="" method="POST">
              <input type="hidden" name="cid" value="<?php echo $arr['deitcian_id']; ?>">
              <td style=" padding:15px 10px 15px 10px; color: black;"><?php echo $arr['deitcian_id']; ?></td>
...
<?php
      ...
      //some particular centre
        ...
        $sql="SELECT * FROM deitcian WHERE centre_id='$centre' ORDER BY '$sort'";
        $run=mysqli_query($db, $sql);
          $arr=mysqli_fetch_array($run);
          //check query run
          if($arr){
...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD']=='POST') {
    if (isset($_POST['view'])) {
      $deitcian_id=$_POST['deitcian_id'];
      $sql="SELECT * FROM deitcian WHERE deitcian_id='$deitcian_id'";
      $run=mysqli_query($db, $sql);
        $arr=mysqli_fetch_array($run);
        if($arr){
?>
      <table>
        <form action="" method="POST">
          <input type="hidden" name="deitcian_id" value="<?php echo $deitcian_id; ?>">
          <tr>
            <td style="font-size: 20px; font-family: cursive; color:#004d00; padding:15px 15px 15px 15px;">deitcian ID</td>
            <td style=" padding:15px 10px 15px 10px; color: black;"><?php echo $arr['deitcian_id']; ?></td>
          </tr>
          ...
          <tr>
            ...
            <td style=" padding:15px 10px 15px 10px; color: black;"><input type="text" name="deitcian_name" value="<?php echo $arr['deitcian_name']; ?>"></td>
          </tr>
          <tr>
...
<?php 
        $sql1="SELECT * FROM customer inner join deitcian  on deitcian.deitcian_id=customer.deitcian_id WHERE customer.deitcian_id='$deitcian_id'";
