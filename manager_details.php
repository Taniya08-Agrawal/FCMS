...
define('DB_USEmname', 'root');
define('DB_PASSWORD', '');
define('DB_DATABASE', 'fitness_centre_management_system');
$db = mysqli_connect(DB_SERVER,DB_USEmname,DB_PASSWORD,DB_DATABASE);
if ($_SERVER['REQUEST_METHOD']=='POST') {
  //update
    if(isset($_POST['update'])){
      $manager_id=$_POST['rid'];
      $mgr_name=$_POST['mname'];
      ...
      $sql="UPDATE manager SET mgr_name='$mgr_name', Contact_No='$Contact_No', address='$address', DOB='$DOB', Gender='$Gender', salary='$salary', joining_date='$joining_date' WHERE manager_id='$manager_id'";
...
?>
    ...
    <label for="condition">
      <b style="font-size: 18px; font-family: cursive; color:#004d00;">Choose filter*</b>
    </label>
    <select name="condition" required>
      <option value="none">None</option>
      <option value="salary_greater">Salary (greater than)</option>
    </select>
...
?>
  <table>
    <tr>
      <td style="font-size: 20px; font-family: cursive; color:#004d00; padding:15px 15px 15px 15px;">manager ID</td>
...
?>
            <tr>
              <form action="" method="POST">
                <input type="hidden" name="rid" value="<?php echo $arr['manager_id']; ?>">
                <td style=" padding:15px 10px 15px 10px; color: black;"><?php echo $arr['manager_id']; ?></td>
                <td style=" padding:15px 10px 15px 10px; color: black;"><?php echo $arr['centre_id']; ?></td>
                <td style=" padding:15px 10px 15px 10px; color: black;"><input type="text" name="mname" value="<?php echo $arr['mgr_name']; ?>"></td>
                ...
                <td ><button type="submit" onclick="typeWriter()" class="submitbtn"  name="update">Update</button></td>
...
<?php
          ...
          //salary greater
            ...
            $sql="SELECT * FROM manager WHERE salary>'$filter_value' ORDER BY salary";
          $run=mysqli_query($db, $sql);
            $arr=mysqli_fetch_array($run);
            //check query run
            if($arr){
...
<?php
        ...
        //some specific centre
          ...
          if($condition=="none"){
          $sql="SELECT * FROM manager WHERE centre_id='$centre'";
          $run=mysqli_query($db, $sql);
            $arr=mysqli_fetch_array($run);
            //check query run
            if($arr){
...
<?php
          ...
          //salary greater
            ...
            $sql="SELECT * FROM manager WHERE salary>'$filter_value' AND centre_id='$centre' ORDER BY salary";
          $run=mysqli_query($db, $sql);
            $arr=mysqli_fetch_array($run);
            //check query run
            if($arr){
